...
class Gaussian:
    ...
    @property
    def get_xyz(self):
        # return self._xyz * self.aabb[None, 3:] + self.aabb[None, :3] ###@chh
        return self._xyz
    ...
    def transform_gs(self,  transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]]):
        if transform is not None:
            transform = np.array(transform)
            xyz = self.get_xyz.detach().cpu().numpy()
            xyz = np.matmul(xyz, transform)
            rotation = utils3d.numpy.quaternion_to_matrix(self.get_rotation.detach().cpu().numpy())
            rotation = np.matmul(rotation, transform)
            rotation = utils3d.numpy.matrix_to_quaternion(rotation)

            self.from_xyz(torch.tensor(xyz, dtype=torch.float, device=self.device))
            self.from_rotation(torch.tensor(rotation, dtype=torch.float, device=self.device))

    # translate the gaussian
    def translate_gs(self,  translate=[0, 0, 0]):
        if translate is not None:
            translate = np.array(translate)
            xyz = self.get_xyz.detach().cpu().numpy() + translate
            # self._scaling = self._scaling 
            self.from_xyz(torch.tensor(xyz, dtype=torch.float, device=self.device))
            
    
    def save_ply(self, path, transform=[[1, 0, 0], [0, 1, 0], [0, 0, 1]]):
        xyz = self.get_xyz.detach().cpu().numpy()
    ...
    # combine all gaussians in the list and return the combined gaussian
    def combine(self, gaussians_list):
        xyz = torch.cat([g._xyz for g in gaussians_list], dim=0)
        features_dc = torch.cat([g._features_dc for g in gaussians_list], dim=0)
        if self.sh_degree > 0:
            features_rest = torch.cat([g._features_rest for g in gaussians_list], dim=0)
        else:
            features_rest = None
        scaling = torch.cat([g._scaling for g in gaussians_list], dim=0)
        rotation = torch.cat([g._rotation for g in gaussians_list], dim=0)
        opacity = torch.cat([g._opacity for g in gaussians_list], dim=0)
