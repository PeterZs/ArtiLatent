...
def load_object(object_path: str) -> None:
    ...
    if file_extension == "blend":
        ...
        # 让每个 part 在 Blender 里有独立的 collection
        for part_idx, obj_path_list in enumerate(object_paths):
            # 新建一个 collection
            part_collection = bpy.data.collections.new(f"part_{part_idx}")
            bpy.context.scene.collection.children.link(part_collection)
            for path in obj_path_list:
                bpy.ops.import_scene.obj(filepath=path)
                # 把刚刚导入的 object 移到 part_collection
                imported_objs = [obj for obj in bpy.context.selected_objects if obj.type == "MESH"]
                for obj in imported_objs:
                    scene_collection = bpy.context.scene.collection
                    if obj.name in scene_collection.objects:
                        scene_collection.objects.unlink(obj)
                    part_collection.objects.link(obj)
...
def main(arg):
    ...
    if arg.object.endswith(".blend"):
        ...
        init_scene()
        num_parts = load_object(arg.object) ##@chh: 返回每个 object 的 part 数量
        if arg.split_normal:
            split_mesh_normal()
    ...
    # @chh: load articulation
    articulation_list = []
    articulation_list = load_articulation(arg.articulation_path)
    
    # for each state, render the scene, and save the camera parameters and images
    views = json.loads(arg.views)
    for state in range(arg.num_states):
        # get the part_collection
        for part_idx in range(num_parts):
            collection_name = f"part_{part_idx}"
            part_collection = bpy.data.collections[collection_name]
            part_meshes = [obj for obj in part_collection.objects if obj.type == "MESH"]
            ...
            if state == 0:
                ...
                if jtype == joint_ref['fwd']['fixed']:
                    ...
                    axis = Vector(axis_d).normalized()
                    center = Vector(axis_o)
                    angle_rad = np.deg2rad(jrange[1] / (arg.num_states))
                    T1 = Matrix.Translation(-center)
                    R = Matrix.Rotation(angle_rad, 4, axis)
                    T2 = Matrix.Translation(center)
                    M = T2 @ R @ T1
                elif jtype == joint_ref['fwd']['prismatic']:
                    axis = Vector(axis_d).normalized()
                    distance = jrange[1] / (arg.num_states)
                    M = Matrix.Translation(axis * distance)
                elif jtype == joint_ref['fwd']['screw']:
                    axis = Vector(axis_d).normalized()
                    center = Vector(axis_o)
                    dist = jrange[0] / (arg.num_states)  # 假设 jrange[0] 是平移量
                    theta = np.deg2rad(jrange[1] / (arg.num_states))  # 假设 jrange[1] 是角度
                    T1 = Matrix.Translation(-center)
                    R = Matrix.Rotation(theta, 4, axis)
                    T2 = Matrix.Translation(center)
                    T3 = Matrix.Translation(axis * dist)
                    M = T3 @ T2 @ R @ T1
                elif jtype == joint_ref['fwd']['continuous']:
                    axis = Vector(axis_d).normalized()
                    center = Vector(axis_o)
                    theta = np.deg2rad(jrange[1] / (arg.num_states))  # 或者用动画帧数
                    T1 = Matrix.Translation(-center)
                    R = Matrix.Rotation(theta, 4, axis)
                    T2 = Matrix.Translation(center)
                    M = T2 @ R @ T1
        ...
        for i, view in enumerate(views):
            ...
            bpy.context.scene.render.filepath = os.path.join(arg.output_folder, f'{state:03d}_{i:03d}.png')
            for name, output in outputs.items():
                output.file_slots[0].path = os.path.join(arg.output_folder, f'{state:03d}_{i:03d}_{name}')
            ...
            metadata = {
                "file_path": f'{state:03d}_{i:03d}.png',
    ...
    # only save mesh for the first state
    # if state == 0:
    if arg.save_mesh:
        # triangulate meshes
        unhide_all_objects()
        ...
        # export ply mesh
        bpy.ops.export_mesh.ply(filepath=os.path.join(arg.output_folder, f'mesh_{state:03d}_{i:03d}.ply'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_states', type=int, help='Number of states to render.')
    ...
    parser.add_argument('--articulation_path', type=str, help='Path to the articulation file.')
