    ...
    try:
        with ThreadPoolExecutor(max_workers=32) as loader_executor, \
            ...
            for _ in tqdm(range(len(sha256s)), desc="Extracting latents"):
                ...
                latent = encoder(feats, sample_posterior=False) #torch.Size([1, 8])
                # import pdb; pdb.set_trace()
                print(f"------------------Latent shape: {latent.shape}")
                assert torch.isfinite(latent.feats).all(), "Non-finite latent"
                pack = {
                    'feats': latent.feats.cpu().numpy().astype(np.float32), #torch.Size([15690, 8])
                    'coords': latent.coords[:, 1:].cpu().numpy().astype(np.uint8), #torch.Size([15690, 4])
                }
